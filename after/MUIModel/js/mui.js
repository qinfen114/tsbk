...
var ip = 'http://111.231.16.133:8844/qinfen'
var user_id = localStorage.getItem('user_id');

function before(id, html) {
	document.getElementById(id).insertAdjacentHTML('beforeend', html)
}

function to(url, id, name) {
	if (id) {
...
function photo(e) {
	return new Promise(imge => {
		var file1 = new FormData();
		file1.append('file', e)
		console.log(file1);
		mui.ajax(ip + '/common/upload', {
			data: file1,
			dataType: 'file', //服务器返回json格式数据
			contentType: false,
			processData: false,
			headers: {
				"Authorization": token
...
function backnew() {
	var ts = plus.webview.currentWebview().opener()
	mui.fire(ts, 'ts')
}

function api(url, type1, data1) {
	return new Promise((res, rej) => {
		mui.ajax(ip + url, {
			data: data1,
			headers: {
				'Content-Type': type1 == 'get' ? '' : 'application/json',
...
var mui = (function(document, undefined) {
	...
	$.qsa = function(selector, context) {
		...
		return $.slice.call(classSelectorRE.test(selector) ? context.getElementsByClassName(RegExp.$1) :
			tagSelectorRE.test(selector) ? context.getElementsByTagName(selector) : context
	...
	var class2type = {};
	$.each(['Boolean', 'Number', 'String', 'Function', 'Array', 'Date', 'RegExp', 'Object', 'Error'], function(
		i, name) {
...
(function($) {
	...
	var delegateFn = function(element, event, selector, callback) {
		return function(e) {
			...
			$.each(handlerQueue, function(index, handler) {
				...
				if (event === 'tap' && (tagName !== 'INPUT' && tagName !== 'TEXTAREA' && tagName !==
						'SELECT')) {
					e.preventDefault();
	...
	$.fn.on = function(event, selector, callback) { //仅支持简单的事件委托,主要是tap事件使用，类似mouse,focus之类暂不封装支持
		return this.each(function() {
			...
			var delegateCallbacks = delegateCallbackObjs[selector] || (delegateCallbackObjs[
				selector] = []);
	...
	$.fn.off = function(event, selector, callback) {
		return this.each(function() {
			...
			if (!event) { //mui(selector).off();
				...
				var delegateCallbacks = delegates[_mid] && delegates[_mid][event] && delegates[_mid][
					event
				][selector];
...
(function($, window, document) {
	...
	window.addEventListener('click', function(
		event) { //解决touch与click的target不一致的问题(比如链接边缘点击时，touch的target为html，而click的target为A)
		var target = event.target;
		var isFound = false;
		for (; target && target !== document; target = target.parentNode) {
			if (target.tagName === 'A') {
...
(function(window) {
	if (!window.requestAnimationFrame) {
		...
		window.cancelAnimationFrame = window.webkitCancelAnimationFrame || window
			.webkitCancelRequestAnimationFrame || function(id) {
				clearTimeout(id);
			};
	};
}(window));
...
(function($, window, name) {
	...
	var dispatchEvent = function(event) {
		...
		if (targetElement) {
			...
			clickEvent.initMouseEvent('click', true, true, window, 1, touch.screenX, touch.screenY, touch
				.clientX, touch.clientY, false, false, false, false, 0, null);
...
(function($, document) {
	$(function() {
		...
		document.addEventListener('focusin', function(e) {
			...
			var target = e.target;
			//TODO 需考虑所有键盘弹起的情况
			if (target.tagName && (target.tagName === 'TEXTAREA' || (target.tagName === 'INPUT' && (
				...
				for (; target && target !== document; target = target.parentNode) {
					...
					if (classList && classList.contains(CLASS_BAR_TAB) || classList.contains(
							CLASS_BAR_FOOTER) || classList.contains(CLASS_BAR_FOOTER_SECONDARY) ||
						classList.contains(CLASS_BAR_FOOTER_SECONDARY_TAB)) {
						isFooter = true;
						break;
					}
				}
...
(function($, window) {
	...
	var getTouches = function(event, touch) {
		...
		touch.isFinal = ((type === $.EVENT_END || type === $.EVENT_CANCEL) && (touchesLength -
	...
	window.addEventListener($.EVENT_CLICK, function(e) {
		...
		if (($.os.android || $.os.ios) && (($.targets.popover && e.target === $.targets.popover) || ($
				.targets.tab) || $.targets.offcanvas || $.targets.modal)) {
			e.preventDefault();
		}
	}, true);
...
(function($, name) {
	...
	var handle = function(event, touch) {
		...
		switch (event.type) {
			...
			case $.EVENT_CANCEL:
				touch.flick = false;
				if (session.flickStart && options.flickMaxTime > (now - flickStartTime) && touch.distance >
					options.flickMinDistince) {
					touch.flick = true;
...
(function($, name) {
	var handle = function(event, touch) {
		...
		if (event.type === $.EVENT_END || event.type === $.EVENT_CANCEL) {
			...
			touch.swipe = false;
			//TODO 后续根据velocity计算
			if (touch.direction && options.swipeMaxTime > touch.deltaTime && touch.distance > options
				.swipeMinDistince) {
				touch.swipe = true;
				$.trigger(session.target, name, touch);
				$.trigger(session.target, name + touch.direction, touch);
	...
	var handle = function(event, touch) {
		...
		switch (event.type) {
			case $.EVENT_END:
				...
				if (!target || (target.disabled || (target.classList && target.classList.contains(
						'mui-disabled')))) {
					return;
				}
				if (touch.distance < options.tapMaxDistance && touch.deltaTime < options.tapMaxTime) {
					if ($.options.gestureConfig.doubletap && lastTarget && (lastTarget ===
							target)) { //same target
...
(function($) {
	...
	$.openWindow = function(url, id, options) {
		...
		if (webview) { //已缓存
			...
			if (webviewCache) {
				webviewCache.afterShowMethodName && webview.evalJS(webviewCache.afterShowMethodName + '(\'' +
			...
			if (nShow.autoShow) {
				var showWebview = function() {
					...
					options.afterShowMethodName && webview.evalJS(options.afterShowMethodName + '(\'' + JSON
						.stringify(params) + '\')');
				};
						...
						$.each(options.subpages, function(index, subpage) {
							...
							if (subpageId) { //过滤空对象
								...
								if (!subWebview) { //如果该webview不存在，则创建
									subWebview = plus.webview.create(subpage.url, subpageId, $
										.windowOptions(subpage.styles), $.extend({
											preload: true
										}, subpage.extras));
								}
			...
			if (isCreate !== false) { //直接创建非预加载窗口
				...
				if (options.subpages) {
					$.each(options.subpages, function(index, subpage) {
						...
						var subWebview = plus.webview.getWebviewById(subpageId);
						if (!subWebview) {
							subWebview = plus.webview.create(subpage.url, subpageId, $.windowOptions(
								subpage.styles), subpage.extras);
						}
						webview.append(subWebview);
					});
				}
		...
		handle: function() {
			...
			if ($.os.plus) {
				$.plusReady(function() {
					...
					//判断是否首页
					if (plus.webview.currentWebview() === plus.webview.getWebviewById(plus
							.runtime.appid)) {
						//首页需要自己激活预加载；
...
(function($) {
	$.addInit({
		...
		handle: function() {
			...
			var hasPulldown = pullRefreshOptions.down && pullRefreshOptions.down.hasOwnProperty(
				'callback');
								...
								$.plusReady(function() {
									...
									if (window.__NWin_Enable__ === false) { //不支持多webview
										...
										if (hasPullup) {
											...
											upOptions.webviewId = webview.id || webview
												.getURL();
...
(function($, window, undefined) {
	...
	$.ajax = function(url, options) {
		...
		if (location.protocol === 'file:' && $.os.ios && window.webkit && window.webkit.messageHandlers && !(
				xhr instanceof plus.net.XMLHttpRequest)) {
		...
		$.ajaxSettings = $.extend($.ajaxSettings, {
			xhr: function(settings) {
				...
				if (originAnchor.protocol !== 'file:') {
					...
					settings.crossDomain = (originAnchor.protocol + '//' + originAnchor
						.host) !== (urlAnchor.protocol + '//' + urlAnchor.host);
					if (settings.crossDomain) {
						return new plus.net.XMLHttpRequest();
					}
				}
				if ($.os.ios && window.webkit && window.webkit
					.messageHandlers) { //wkwebview下同样使用5+ xhr
					return new plus.net.XMLHttpRequest();
				}
				return new window.XMLHttpRequest();
			}
...
(function($, document, undefined) {
	...
	var PullRefresh = {
		...
		_createPocket: function(clazz, options, iconClass) {
			...
			pocket.innerHTML = pocketHtml.replace('{contentrefresh}', options.contentinit).replace('{icon}',
				iconClass);
			return pocket;
		},
...
(function($, window, document, undefined) {
	...
	var Scroll = $.Class.extend({
		...
		_nearestSnap: function(x) {
			...
			for (; i < length; i++) {
				var nearestX = this.direction === 'left' ? this.pages[i][0].leftX : this.pages[i][0]
					.rightX;
				if (x >= nearestX) {
		...
		_drag: function(e) {
			...
			//			}
			var detail = e.detail;
			if (this.options.scrollY || detail.direction === 'up' || detail.direction ===
				'down') { //如果是竖向滚动或手势方向是上或下
				//ios8 hack
		...
		_flick: function(e) {
			...
			if (this.options.momentum && detail.flickTime < 300) {
				momentumX = this.hasHorizontalScroll ? this._momentum(this.x, detail.flickDistanceX,
					detail.flickTime, this.maxScrollX, this.options.bounce ? this.wrapperWidth : 0,
					this.options.deceleration) : {
					destination: newX,
					duration: 0
				};
				momentumY = this.hasVerticalScroll ? this._momentum(this.y, detail.flickDistanceY,
					detail.flickTime, this.maxScrollY, this.options.bounce ? this.wrapperHeight : 0,
					this.options.deceleration) : {
					destination: newY,
					duration: 0
				};
		...
		_scrollend: function(e) {
			if ((this.y === 0 && this.maxScrollY === 0) || (Math.abs(this.y) > 0 && this.y <= this
					.maxScrollY)) {
		...
		setTranslate: function(x, y) {
			...
			if (this.parallaxElement && this.options.scrollY) { //目前仅支持竖向视差效果
				...
				if (scale > 1) {
					...
					this.parallaxStyle['webkitTransform'] = this._getTranslateStr(0, -parallaxY) +
						' scale(' + scale + ',' + scale + ')';
				} else {
	...
	var Indicator = function(scroller, options) {
		...
		if (this.options.fade) {
			...
			this.wrapperStyle['webkitTransitionDuration'] = this.options.fixedBadAndorid && $.os.isBadAndroid ?
				'0.001s' : '0ms';
	...
	Indicator.prototype = {
		...
		refresh: function() {
			...
			if (this.options.listenX && !this.options.listenY) {
				...
				this.indicatorStyle.display = this.scroller.hasHorizontalScroll || this.scroller
					.hasVerticalScroll ? 'block' : 'none';
			}
			...
			if (this.options.listenX) {
				...
				this.indicatorWidth = Math.max(Math.round(this.wrapperWidth * this.wrapperWidth / (this
					.scroller.scrollerWidth || this.wrapperWidth || 1)), 8);
				...
				this.sizeRatioX = this.options.speedRatioX || (this.scroller.maxScrollX && (this.maxPosX /
					this.scroller.maxScrollX));
			}

			if (this.options.listenY) {
				this.wrapperHeight = this.wrapper.clientHeight;
				this.indicatorHeight = Math.max(Math.round(this.wrapperHeight * this.wrapperHeight / (this
					.scroller.scrollerHeight || this.wrapperHeight || 1)), 8);
				...
				this.sizeRatioY = this.options.speedRatioY || (this.scroller.maxScrollY && (this.maxPosY /
	...
	var PullRefresh = $.Scroll.extend($.extend({
		...
		_start: function(e) {
			...
			if (e.touches && e.touches.length && e.touches[0].clientX > 30) {
				e.target && !this._preventDefaultException(e.target, this.options
					.preventDefaultException) && e.preventDefault();
			}
			if (!this.loading) {
		...
		_drag: function(e) {
			...
			this._super(e);
			if (!this.pulldown && !this.loading && this.topPocket && e.detail.direction ===
				'down' && this.y >= 0) {
				this._initPulldownRefresh();
			}
			if (this.pulldown) {
				this._setCaption(this.y > this.options.down.height ? this.options.down.contentover :
...
(function($, window) {
	...
	var Slider = $.Slider = $.Scroll.extend({
			...
			if (indicatorWrap) {
				...
				if (indicators.length > 0) { //图片轮播
					for (var i = 0, len = indicators.length; i < len; i++) {
						indicators[i].classList[i === detail.slideNumber ? 'add' : 'remove'](
							CLASS_ACTIVE);
					}
				} else {
					...
					if (number) { //图文表格
						...
						for (var i = 0, len = controlItems.length; i < len; i++) {
							controlItems[i].classList[i === detail.slideNumber ? 'add' : 'remove'](
								CLASS_ACTIVE);
						}
					}
				}
		...
		_handleIndicatorTap: function(event) {
			var self = this;
			var target = event.target;
			if (target.classList.contains(CLASS_ACTION_PREVIOUS) || target.classList.contains(
					CLASS_ACTION_NEXT)) {
		...
		_flick: function(e) {
			...
			if (e.type === 'flick') {
				if (detail.deltaTime < 200) { //flick，太容易触发，额外校验一下deltaTime
					this.x = this._getPage((this.slideNumber + (direction === 'right' ? -1 : 1)), true)
						.x;
				}
				this.resetPosition(this.options.bounceTime);
		...
		_initSnap: function() {
			...
			if (!this.currentPage.x) {
				...
				this.slideNumber = 0;
				this.lastSlideNumber = typeof this.lastSlideNumber === 'undefined' ? 0 : this
					.lastSlideNumber;
			} else {
				this.slideNumber = this._fixedSlideNumber();
				this.lastSlideNumber = typeof this.lastSlideNumber === 'undefined' ? this.slideNumber :
		...
		_gotoItem: function(slideNumber, time) {
			this.currentPage = this._getPage(slideNumber,
				true); //此处传true。可保证程序切换时，动画与人手操作一致(第一张，最后一张的切换动画)
		...
		setTranslate: function(x, y) {
			...
			if (progressBar) {
				this.progressBarStyle.webkitTransform = this._getTranslateStr((-x * (this
	...
	$.plusReady(function() {
		...
		var PlusPullRefresh = $.Class.extend({
			...
			_init: function() {
				...
				self.scrollInterval = window.setInterval(function() {
					if (self.isScroll && !self.loading) {
						if (window.pageYOffset + window.innerHeight + 10 >= document
							.documentElement.scrollHeight) {
							self.isScroll =
								false; //放在这里是因为快速滚动的话，有可能检测时，还没到底，所以只要有滚动，没到底之前一直检测高度变化
				...
				$.plusReady(function() {
					if (self.options.down.style == "circle") {
						...
						webview.addEventListener('close', function() {
							var attrWebviewId = self.options.webviewId && self
								.options.webviewId.replace(/\//g,
									"_"); //替换所有"/" 
							self.element.removeAttribute(
								'data-pullrefresh-plus-' + attrWebviewId);
						});
			...
			_pulldownLoading: function() { //该方法是父页面调用的
				var self = this;
				$.plusReady(function() {
					var childWebview = plus.webview.getWebviewById(self.options
						.webviewId);
			...
			_endPulldownToRefresh: function() { //该方法是父页面调用的
				...
				if (self.topPocket && self.options.webview) {
					...
					self._setCaption(self.options.down.contentdown, true);
					setTimeout(function() {
						self.loading || self.topPocket.classList.remove(
							CLASS_BLOCK);
					}, 350);
				}
			},
			beginPullup: function(callback) { //开始上拉加载
				...
				setTimeout(function() {
					...
					self.pullCaption.innerHTML =
						''; //修正5+里边第一次加载时，文字显示的bug(还会显示出来个“多”,猜测应该是渲染问题导致的)
					self.pullCaption.className = CLASS_PULL_CAPTION + ' ' +
						CLASS_PULL_CAPTION_REFRESH;
			...
			disablePulldownToRefresh: function() {
				var webview = plus.webview.currentWebview();
				if (this.options.down.style && this.options.down.style ==
					'circle') { // 单webview模式禁止原生下拉刷新
			...
			enablePullupToRefresh: function() {
				this._initPullupRefresh();
				this.bottomPocket.classList.remove(CLASS_HIDDEN);
				this.pullCaption.className = CLASS_PULL_CAPTION + ' ' +
					CLASS_PULL_CAPTION_DOWN;
		...
		$.fn.pullRefresh_native = function(options) {
			...
			!options.webviewId && (options.webviewId = (plus.webview.currentWebview().id || plus.webview
	...
	var OffCanvas = $.Class.extend({
		...
		refresh: function(offCanvas) {
			...
			if (this.offCanvasRight) {
				...
				this.offCanvasRightSlideIn = this.slideIn && (this.offCanvasRight.parentNode === this
					.wrapper);
			...
			if (this.offCanvasLeft) {
				...
				this.offCanvasLeftSlideIn = this.slideIn && (this.offCanvasLeft.parentNode === this
					.wrapper);
		...
		handleEvent: function(e) {
			switch (e.type) {
				...
				case 'drag':
					...
					if (!this.isDragging && Math.abs(this.lastX - this.startX) > this.options
						.dragThresholdX && (detail.direction === 'left' || (detail.direction ===
							'right'))) {
						if (this.slideIn) {
							this.scroller = this.wrapper.querySelector(SELECTOR_INNER_WRAP);
							if (this.classList.contains(CLASS_ACTIVE)) {
								if (this.offCanvasRight && this.offCanvasRight.classList.contains(
										CLASS_ACTIVE)) {
					...
					if (this.isDragging) {
						...
						if (!this.slideIn) {
							if (x >= 0) {
								...
								ratio = (this.offCanvasRightWidth && (x / this.offCanvasRightWidth)) ||
									0;
							}
							if (ratio === 0) {
							...
							if (direction === 'right' && ratio >= 0 && (ratio >= 0.5 || detail
									.swipe)) { //右滑打开
								this.openPercentage(100);
							} else if (direction === 'right' && ratio < 0 && (ratio > -0.5 || detail
									.swipe)) { //右滑关闭
								this.openPercentage(0);
								...
								this.openPercentage(-100);
							} else if (direction === 'left' && ratio <= 0 && (ratio <= -0.5 || detail
									.swipe)) { //左滑打开
								this.openPercentage(-100);
							} else if (direction === 'left' && ratio > 0 && (ratio <= 0.5 || detail
									.swipe)) { //左滑关闭
		...
		setTranslateX: $.animationFrame(function(x) {
			if (this.scroller) {
				if (this.scalable && this.offCanvas.parentNode === this.wrapper) {
					...
					this.offCanvas.style.webkitTransform = 'translate3d(0,0,0) scale(' +
						zoomInScale + ')';
					this.scroller.style.webkitTransform = 'translate3d(' + x + 'px,0,0) scale(' +
						zoomOutScale + ')';
				} else {
					if (this.slideIn) {
		...
		isShown: function(direction) {
			...
			if (!this.slideIn) {
				...
				if (direction === 'left') {
					shown = this.classList.contains(CLASS_ACTIVE) && this.wrapper.querySelector('.' +
					...
					shown = this.classList.contains(CLASS_ACTIVE) && (this.wrapper.querySelector('.' +
							CLASS_OFF_CANVAS_LEFT + '.' + CLASS_ACTIVE) || this.wrapper
						.querySelector('.' + CLASS_OFF_CANVAS_RIGHT + '.' + CLASS_ACTIVE));
				}
			}
			return shown;
		},
		close: function() {
			this._initOffCanvasVisible();
			this.offCanvas = this.wrapper.querySelector('.' + CLASS_OFF_CANVAS_RIGHT + '.' +
				CLASS_ACTIVE) || this.wrapper.querySelector('.' + CLASS_OFF_CANVAS_LEFT + '.' +
				CLASS_ACTIVE);
		...
		toggle: function(directionOrOffCanvas) {
			...
			if (directionOrOffCanvas && directionOrOffCanvas.classList) {
				direction = directionOrOffCanvas.classList.contains(CLASS_OFF_CANVAS_LEFT) ? 'left' :
					'right';
...
(function($, window, document, name) {
	...
	var removeBackdrop = function(popover) {
		...
		removeBackdropTimer = $.later(function() {
			if (!popover.classList.contains(CLASS_ACTIVE) && backdrop.parentNode && backdrop
	...
	var togglePopover = function(popover, anchor, state) {
		if ((state === 'show' && popover.classList.contains(CLASS_ACTIVE)) || (state === 'hide' && !popover
		...
		if (popover.classList.contains(CLASS_BAR_POPOVER) || popover.classList.contains(
				CLASS_ACTION_POPOVER)) { //navBar
...
(function($, window, document, name, undefined) {
	...
	var handle = function(event, target) {
		if (target.classList && (target.classList.contains(CLASS_CONTROL_ITEM) || target.classList.contains(
				CLASS_TAB_ITEM))) {
	...
	window.addEventListener('tap', function(e) {
		...
		for (; segmentedControl && segmentedControl !== document; segmentedControl = segmentedControl
			.parentNode) {
...
(function($, window, name) {
	...
	Toggle.prototype.setTranslateX = $.animationFrame(function(x) {
		...
		var isChanged = false;
		if ((this.initialState && -x > (this.handleX / 2)) || (!this.initialState && x > (this.handleX /
				2))) {
			isChanged = true;
		}
		if (this.lastChanged !== isChanged) {
			if (isChanged) {
				this.handle.style.webkitTransform = 'translate(' + (this.initialState ? 0 : this.handleX) +
					'px,0)';
				this.classList[this.initialState ? 'remove' : 'add'](CLASS_ACTIVE);
			} else {
				this.handle.style.webkitTransform = 'translate(' + (this.initialState ? this.handleX : 0) +
					'px,0)';
...
(function($, window, document) {
		...
		if (translateX !== lastTranslateX) {
			if (buttonsRight && buttonsRight.length > 0) {
				...
				if (translateX < -sliderActionRightWidth) {
					translateX = -sliderActionRightWidth - Math.pow(-translateX - sliderActionRightWidth,
						overFactor);
				}
			...
			if (buttonsLeft && buttonsLeft.length > 0) {
				...
				if (translateX > sliderActionLeftWidth) {
					translateX = sliderActionLeftWidth + Math.pow(translateX - sliderActionLeftWidth,
						overFactor);
				}
				for (var i = 0, len = buttonsLeft.length; i < len; i++) {
					var buttonLeft = buttonsLeft[i];
					if (typeof buttonLeft._buttonOffset === 'undefined') {
						buttonLeft._buttonOffset = sliderActionLeftWidth - buttonLeft.offsetLeft - buttonLeft
							.offsetWidth;
					}
		...
		for (; target && target !== document; target = target.parentNode) {
			if (target.classList) {
				...
				if ((target.tagName === 'INPUT' && target.type !== 'radio' && target.type !== 'checkbox') ||
					target.tagName === 'BUTTON' || classList.contains(CLASS_TOGGLE) || classList.contains(
						CLASS_BTN) || classList.contains(CLASS_DISABLED)) {
					isDisabled = true;
				}
				...
				if (classList.contains(CLASS_TABLE_VIEW_CELL)) {
					...
					if (!cell.parentNode.classList.contains(CLASS_RADIO_VIEW) && selected && selected !==
						cell) {
	...
	var handleEvent = {
		...
		drag: function(event) {
			...
			if (!isMoved) { //init
				sliderHandle = sliderActionLeft = sliderActionRight = buttonsLeft = buttonsRight =
					sliderDirection = sliderRequestAnimationFrame = false;
				...
				if (sliderHandle) {
					...
					if (isOpened) {
						openedActions = cell.querySelector(SELECTOR_SLIDER_LEFT + SELECTOR_SELECTED) ?
							'left' : 'right';
					}
				}
			}
		...
		dragend: function(event) {
			...
			isMoved = false;
			var action = 'close';
			var actionsWidth = sliderDirection === 'toLeft' ? sliderActionRightWidth :
			...
			if (buttonsLeft && buttonsLeft.length > 0 && buttonsLeft !== buttons) {
				for (var i = 0, len = buttonsLeft.length; i < len; i++) {
					...
					if (typeof buttonOffset === 'undefined') {
						buttonLeft._buttonOffset = sliderActionLeftWidth - buttonLeft.offsetLeft -
							buttonLeft.offsetWidth;
					}
	...
	window.addEventListener($.EVENT_CANCEL, function(
		event) { //使用touchcancel来取消高亮，避免一次点击既不触发tap，doubletap，longtap的事件
		if (!cell) {
			return;
		}
		toggleActive(false);
...
(function($, window, document) {
	...
	var createInput = function(placeholder) {
		return '<div class="' + CLASS_POPUP_INPUT + '"><input type="text" autofocus placeholder="' + (
			placeholder || '') + '"/></div>';
	};
	...
	var createButtons = function(btnArray) {
		...
		var btns = [];
		for (var i = 0; i < length; i++) {
			btns.push('<span class="' + CLASS_POPUP_BUTTON + (i === length - 1 ? (' ' +
				CLASS_POPUP_BUTTON_BOLD) : '') + '">' + btnArray[i] + '</span>');
		}
	...
	var createPopup = function(html, callback) {
		...
		popupElement.addEventListener('webkitTransitionEnd', function(e) {
			if (popupElement && e.target === popupElement && popupElement.classList.contains(
					CLASS_POPUP_OUT)) {
				removePopupElement();
			}
		});
	...
	var createAlert = function(message, title, btnValue, callback, type) {
		...
		if (!$.os.plus || type === 'div') {
			return createPopup(createInner(message, title || '提示') + createButtons([btnValue || '确定']),
				callback);
		}
	...
	var createConfirm = function(message, title, btnArray, callback, type) {
		...
		if (!$.os.plus || type === 'div') {
			return createPopup(createInner(message, title || '提示') + createButtons(btnArray || ['取消', '确认']),
				callback);
		}
	...
	var createPrompt = function(message, placeholder, title, btnArray, callback, type) {
		...
		if (!$.os.plus || type === 'div') {
			return createPopup(createInner(message, title || '提示', createInput(placeholder)) + createButtons(
				btnArray || ['取消', '确认']), callback);
		}
...
(function($, document) {
	...
	var showProgressbar = function(container, progress, color) {
		...
		if (container.classList.contains(CLASS_PROGRESSBAR)) {
			...
			var progressbars = container.querySelectorAll(SELECTOR_PROGRESSBAR + ':not(.' +
				CLASS_PROGRESSBAR_OUT + ')');
			if (progressbars) {
				for (var i = 0, len = progressbars.length; i < len; i++) {
			...
			if (!progressbar) {
				...
				progressbar.className = CLASS_PROGRESSBAR + ' ' + CLASS_PROGRESSBAR_IN + (typeof progress !==
					'undefined' ? '' : (' ' + CLASS_PROGRESSBAR_INFINITE)) + (color ? (' ' +
					CLASS_PROGRESSBAR + '-' + color) : '');
		...
		if (row) {
			if (self.sliderActionClass) {
				...
				if (self.searchActionClass) {
					self.searchAction = self.createAction(row, self.searchActionClass, self
				...
				if (self.speechActionClass) {
					self.speechAction = self.createAction(row, self.speechActionClass, self
				...
				if (self.passwordActionClass) {
					self.passwordAction = self.createAction(row, self.passwordActionClass, self
	...
	Input.prototype.createAction = function(row, actionClass, actionSelector) {
		...
		if (!action) {
			...
			if (actionClass === this.searchActionClass) {
				action.innerHTML = '<span class="' + CLASS_ICON + ' ' + CLASS_ICON_SEARCH + '"></span><span>' +
					this.element.getAttribute('placeholder') + '</span>';
				...
				$.each(['keyup', 'change', 'input', 'focus', 'cut', 'paste'], function(index, type) {
					(function(type) {
						element.addEventListener(type, function() {
							action.classList[element.value.trim() ? 'remove' : 'add'](
								CLASS_HIDDEN);
						});
					})(type);
				});
	...
	var Numbox = $.Numbox = $.Class.extend({
		...
		checkValue: function() {
			...
			if (val == null || val == '' || isNaN(val)) {
				...
				if (self.options.max != null && !isNaN(self.options.max) && val >= parseInt(self.options
						.max)) {
				...
				if (self.options.min != null && !isNaN(self.options.min) && val <= parseInt(self.options
						.min)) {
...
(function($, window, document) {
	...
	Button.prototype.setState = function(state) {
		...
		if (state === STATE_RESET) {
			...
			var html = this.isInput ? this.options.loadingText : ('<span>' + this.options.loadingText +
				'</span>');
