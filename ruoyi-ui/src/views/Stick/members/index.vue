    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['Stick:members:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['Stick:members:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listMembers, getMembers, delMembers, addMembers, updateMembers } from "@/api/Stick/members";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除成员列表编号为"' + uids + '"的数据项？').then(function() {
        return delMembers(uids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('Stick/members/export', {
        ...this.queryParams
